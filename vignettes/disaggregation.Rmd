...
```{r, echo=FALSE}
isINLA <- "INLA" %in% utils::installed.packages()
```
...
**Gaussian** ($\sigma$ is the dispersion of the normal likelihood)

$dnorm(y_j/\sum agg_i, rate_j, \sigma)$
...
The user can also specify the priors for the regression parameters and the iid effect. For the field, the user specifies the pc priors for the range, $\rho_{min}$ and $\rho_{prob}$, where $P(\rho < \rho_{min}) = \rho_{prob}$, and the variation, $\sigma_{min}$ and $\sigma_{prob}$, where $P(\sigma > \sigma_{min}) = \sigma_{prob}$, in the field.
