...
library(sp, quietly = TRUE) # Don't need to read data. So just here while I learn sf.
...
ggplot() + geom_sf(data = df, aes(fill = cases / population))
...
pop_raster <- rasterize(terra::vect(df), cov_stack, field = 'pop_per_cell')
...
data_for_model <- prepare_data(x = df,
                               cov_stack,
                               pop_raster,
                               response_var = 'cases',
